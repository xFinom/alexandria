...
<header class="page" style="background-image: url('{{asset('images/header.jpg')}}');">
    <div class="container">
        <div class="page_title">
            ...
            <h1 class="page_title-title outlined outlined--white" data-aos="fade-left">Buy a Book</h1>
                        ...
                        <li class="shop_list-item">
                                ...
                                <div class="media_slider media_slider--single media_slider--single--1">
                                    <div class="swiper-wrapper">
                                        <div class="swiper-slide">
                                            @php
                                                $path = $item->book->image ? 'storage/' . $item->book->image : 'images/nocover.jpg';
                                            @endphp
                                            <picture>
                                                <source
                                                    data-srcset="{{ $path }}"
                            ...
                            <div class="main">
                                <div class="main_header">
                                    <span class="label">{{$item->book->author}}</span>
                                    <a class="title h4">{{$item->book->title}}</a>
                                    <div class="price h4 d-flex">
                                        <span class="price_item price_item">$ {{$item->price}} MXN</span>
                                    </div>
                                </div>
                                <div class="main_footer">
                                    ...
                                    <ul class="main_footer-list">
                                        <li class="main_footer-list_item">
                                            <i class="icon">- ISBN:</i> {{$item->book->ISBN}}
                                        </li>
                                        <li class="main_footer-list_item">
                                            <i class="icon">- Edition:</i> {{$item->book->edition}}
                                        </li>
                                        <li class="main_footer-list_item">
                                            <i class="icon">- Publisher:</i> {{$item->book->publisher}}
                                        </li>
                                        <li class="main_footer-list_item">
                                            <i class="icon">- Pages:</i> {{$item->book->pages}}
                                        </li>
                                        <li class="main_footer-list_item">
                                            <i class="icon">- Stock:</i> {{$item->stock}}
                                        </li>
                                    </ul>
                                    ...
                                    <div class="wrapper d-flex">
                                        <form action="{{ route('checkout') }}" method="post">
                                            @csrf
                                            <input type="hidden" name="book_inventory_id" id="book_inventory_id" value="{{$item->id}}">
                                            <input type="hidden" name="title" id="title" value="{{$item->book->title}}">
                                            <input type="hidden" name="price" id="price" value="{{$item->price}}">
                                            <button type="submit" class="btn btn--black btn--shop addToCart" href="{{ route('checkout') }}">
